...
**そして読みは「示したもの」を言います。** 分母が薄い軸も、基準線と並んだ軸も、
「測れませんでした」という段落にはなりません——**読みから丸ごと落ち、対話からも外れます**。
弱点でも宿題でもなく、そこでコップが反応しなかっただけ。数字は結果JSONに残ります。
**逆向きに出た軸は別物**として名指しし、強みの列には混ぜません。
これが水見式と「できなかったことの一覧」の違いです。
...
**そして残差が意味を持つのは、5系統が残りの大半を説明できている時だけです。** recall は
**実質発話**（適性を担いうる発話。「うん」「続けて」は数えない）に対して測り、**50%の床**を下回ると
道具は `usable: false` を返します——そこから下では、残差の大半は「検出器が取りこぼしたもの」であって
「5系統が説明できないもの」ではないからです。作った元のコーパスでは実質発話389件に対し **27.2%**。
つまりそこでは特質系は**そもそも認定不可能**で、それを人の判断に委ねず道具が言います。
この数字を上げるのが残っている作業で、動くまで残差は読み物であって証拠ではありません。
...
- **英語パターンは特異度は実測済み、感度は未計測。** 日本語側は実コーパス505件に当てて調整しました。
  ...
  **AIに向けて書かれたわけではない普通の文章に当たってしまわないか（特異度）**。
  ...
  手元の任意の文章に向けて走らせられます。このリポ自身の文書165段落での実測では、
  実務上いちばん緩いのが英語 `constraint` の **9.1%** と `reasoning` の **8.5%**——
  文書は "must" "never" "because" が異常に多いので、これは実際の指示文での率ではなく**上限**として読んでください。
  `concrete` は 80.0% ですが**それが正常**です（「識別子や数字を含むか」を見る否定側のガード）。
