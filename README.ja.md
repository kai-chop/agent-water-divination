...
| 系統 | 固有の軸 | 分母 |
|---|---|---|
| 強化系 | 早期に置いた制約のまま、訂正なしで完走したセッション | セッション |
| 放出系 | AIが「どういう意味か」を訊かずに着手できた依頼 | 依頼 |
| 変化系 | 曖昧な出だしが、他人にも判定できる形になった割合 | 曖昧な出だし |
| 具現化系 | 完了条件つき依頼のうち、AIが実際に証拠を持って戻った割合 | 完了条件つき依頼 |
| 操作系 | ルール・フック・設定へ書き込まれた宣言 | ルール宣言 |
| 特質系 | 率ではない（後述） | — |
...
6つのうち5つは水に、特質系だけは葉に起きる。測り方を変えているのも同じ理由で、
特質系は**稀少事象のカタログ**です。どれも3〜4個の出来事が**順番どおり・近い距離で**
揃って初めて成立します。

- **機構になった学び** — 訂正 → ルール宣言 → そのルールがファイル/フック/設定へ書き込まれる、が一続きで起きた。その失敗は誰かの記憶に依存しなくなった
- **感覚が検査になった** — 「なんか違う」→ 他人が適用できる基準 → AIがそれで検証した
- **別々の出所が1つに編まれた** — 外から持ち込んだ素材と自分の指示が並び、その組み合わせから恒久的な成果物が出た
- **始める前に反証を求めた** — まだ安いうちに反例を要求した

ほとんどの期間で**0件**が正常です。ここでの「稀少」は**定義が厳しいという意味での稀少**であって、
他人と比べて珍しいという意味ではありません。このリポには他者比較のベースラインが無く、
でっち上げれば捏造になります。
...
- **英語パターンは未計測。** 日本語側は実コーパス505件に当てて調整しましたが、`patterns/en.json` は
  その意図を英語に移しただけで、フィクスチャ対照しかありません。英語の数値は日本語より弱い証拠として扱ってください。
